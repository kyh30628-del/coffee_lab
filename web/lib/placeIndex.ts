 ...
 *  ⚠️ 결재 #1086(2026-09-14): 필러어는 원본 문자열에서 **공백으로 둘러싸인 낱말일 때만** 지운다.
 *     이전엔 norm()으로 공백을 먼저 지운 뒤 부분일치로 지워, "커피빈"의 "커피"까지 잘려 "빈"(1자)이
 *     남았다 — searchPlacesExact가 2자 미만은 버려 장소 후보가 통째로 사라지고, 그 여파로 같은 유형의
 *     "스타벅스"(필러어 미포함이라 안 잘림)와 응답이 갈렸다(decisions#1086 결함1). */
export const placeKey = (q: string) => norm(q.replace(/(?:^|\s)(카페|커피|맛집|근처|주변|추천|가볼만한곳|가볼만한)(?=\s|$)/g, " "));
