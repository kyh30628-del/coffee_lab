...
const COMMERCE_JUNK = /(중고\s*거래|직거래|택배\s*(거래|비)|판매\s*(합니다|중입니다|글|가격|처)|구해요|구합니다|분양\s*(합니다|중|가)|새\s*상품|정품\s*인증|택포|계좌\s*(번호|이체)|입금\s*(계좌|확인|자)|견적\s*문의|도매상|인치\s*(모니터|tv|티비)|희망가격|선호\s*거래지역|판매자\s*신뢰|상품\s*설명|거래\s*지역)/i;
