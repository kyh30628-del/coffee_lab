...
const OFFTOPIC_TOPIC = /(미야자와|겐지|그림책|동화책|독서모임|북클럽|독후감|감상문|시집|소설책|에세이집|문학상|월드컵|올림픽|성배전쟁|붕괴\s*\d|원신|게임\s*(이벤트|공략|패치|리뷰)|콘서트\s*후기|앨범\s*리뷰|컴백|뮤직비디오|드라마\s*리뷰|웹툰|넷플릭스|바람에도\s*지지|폭풍에도\s*지지|더위에도\s*지지|눈에도\s*지지|눈보라에도)/;
...
export function verifyReview(input: QualityInput): QualityResult {
  ...
  if (OFFTOPIC_TOPIC.test(fullL) && !CAFE_CONTEXT.test(fullL)) {
    return { verdict: "rejected", score: 3, reasons: ["비카페 주제 글(문학·게임·미디어)"], signals: sig };
  }
