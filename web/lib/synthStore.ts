...
//   문화기획전시·티카페(밀크티·버블티·망고사고) 등 니치 업종 어휘 미등재로 실제 정상 카페 후기의 offctx_rate가
//   과대측정되던 현상(4사이클 연속 재현, 누적 14곳+) — 기조실장 정책판단으로 사전 확장(#599).
// ★ 룰갭발굴팀 1차 사이클(08-31, decisions#916): 호텔 라운지/F&B 어휘 미등재로 정상 방문후기가 offctx 오탐
//   (id25017·9634·25271, 반대방향 — 과다검출). 애프터눈티·르구떼·딤섬·딸기뷔페·하이티·라운지 등 호텔 F&B 어휘 추가.
const CAFE_CTX = /(카페|커피|라떼|아메리카노|에스프레소|콜드브루|핸드드립|디저트|케이크|베이커리|메뉴|음료|원두|바리스타|좌석|매장|사장님|주문|브런치|로스팅|카공|빙수|스무디|에이드|방문|다녀|마셨|마시|들렀|시켰|먹었|cafe|coffee|latte|빵집?|빵|꽈배기|도넛|도나쓰|도나스|마카롱|과자|타르트|휘낭시에|쿠키|스콘|크루아상|크로플|베이글|찹쌀|꿀|크림|초코|티라미수|와플|토스트|샌드위치|간식|아인슈페너|플랫화이트|차\b|티\b|푸딩|젤라또|아이스크림|약과|디카페인|음식|맛있|존맛|JMT|먹스타|팥죽|단팥죽|쌍화탕|한방차|대추차|인삼차|유자차|생강차|오미자|다과|전통차|찻집|티하우스|한방|빙떡|경단|맛집|레스토랑|초콜릿|쇼콜라|봉봉|목장|치즈|요거트|답례품|파충류|재즈|공연|전시|밀크티|버블티|망고사고|애프터눈|르구떼|르\s?꾸떼|딤섬|딸기뷔페|하이티|라운지|해피아워|조식)/i;
...
async function storeResult(cafeId: number, name: string, result: CollectResult, llmJudged: boolean) {
  ...
  //   자동비공개 아님, needs_llm 재판정 큐 우선순위 태그만 부여(반증사례 id14844 TV방영 진짜버즈 보호 확인됨).
