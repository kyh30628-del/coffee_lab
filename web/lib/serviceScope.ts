...
export const OUT_OF_SCOPE_PREFIXES = [
  // ⚠️ 2026-09-06 부산·경남 편입: "경상"·"경남" 제거. "경상" 통접두를 남기면 "경상남도 …" 실주소가
  //   계속 범위 밖으로 차단된다(계획서 09-04 지뢰).
  // 🔴 2026-09-12 대구·경북 편입: "경상북도"·"경북"·"대구" 제거. 09-11 개방 커밋이 regionList.ts만 고치고
  //   이 짝을 놓쳐, 공개 승격된 대구·경북 1,062곳을 autoCorrect가 "서비스 범위 밖 시·도 주소"로
  //   2,222곳 즉시 제외했다(실제 사고). 지역 편입은 **이 파일과 regionList.ts를 반드시 같이** 고친다.
  // 🆕 2026-09-17 전국 개방(CEO "전부 다"): "전라"·"전북"·"전남"·"울산"·"제주"·"광주광역시"를 전부 제거.
  //   → **이 배열이 비었다.** 이제 서비스 범위 밖 시·도는 없다(전국). 배열·SQL 경로는 그대로 두어
  //     다음에 범위를 다시 좁힐 때 여기만 채우면 되게 한다. 빈 배열일 때 SQL은 'false'가 된다(아래).
] as const;
// 서비스 범위 안: **전국**(2026-09-17 광주·전남·전북·울산·제주 편입으로 완성).
//   lib/regionList.ts의 SIDO_GU와 **짝을 이룬다** — 한쪽만 고치면 정상 카페가 "범위 밖 공개"로 오경보 난다
//   (2026-08-26 강원 139곳이 실제로 그렇게 HIGH 경보로 떴다).

/** `address LIKE ...` OR 절 — SQL 조각으로 그대로 끼워 쓴다(태그드 템플릿은 조각 합성이 안 되므로 문자열). */
// ⚠️ 빈 배열이면 join이 ""가 되어 SQL이 깨진다(WHERE 절에 그대로 끼워 쓰므로).
//   전국 개방(2026-09-17)으로 실제로 비었다 → 'false'로 폴백해 "범위 밖인 것은 없다"를 뜻하게 한다.
export const OUT_OF_SCOPE_SQL = OUT_OF_SCOPE_PREFIXES.length
  ? OUT_OF_SCOPE_PREFIXES.map((p) => `address LIKE '${p}%'`).join(" OR ")
  : "false";
