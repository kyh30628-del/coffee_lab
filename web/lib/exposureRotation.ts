...
// KST 기준 '일 인덱스'(1970-01-01 KST=0). 홈 '오늘의 테마' 축 선택·기간 회전에 공용 사용.
export function dayIndexKST(now: number = Date.now()): number {
  return Math.floor((Math.floor(now / 60000) + KST_OFFSET_MIN) / 1440);
}
