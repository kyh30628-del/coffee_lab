...
export const isNonCafe = (name: string, category: string) => {
  ...
  if (cat) {
    // ⚠️ 네이버는 카페를 '음식점>카페,디저트'로 분류한다(카페가 음식점의 하위). 따라서 전체 문자열의 '음식점'으로 판단하면 안 되고,
    //    반드시 마지막 '>' 뒤(리프)로 본다. 리프가 카페류면 '음식점>카페,디저트'여도 카페다.
    const leaf = (cat.split(">").pop() || "").trim();
    // ① 카페류 리프 → 통과(최우선)
    if (/(카페|커피|로스터|찻집|티하우스|북카페)/.test(leaf)) return false;
    // ② 카페형 디저트·베이커리·브런치 → 보수적 통과
    if (/(베이커리|제과|디저트|브런치|도넛|케이크)/.test(leaf)) return false;
    // ③ 간식 단품 전문 리프 → 제외(와플대학·아이스크림 등)
    if (/(와플|토스트|핫도그|샌드위치|샐러드|아이스크림|빙수|젤라또|요거트|츄러스|크레페|타코야끼|붕어빵|호떡|꽈배기|어묵|떡집|방앗간|반찬|정육|청과)/.test(leaf)) return true;
    // ④ 음식점·주점 리프 → 제외
    if (/(음식점|한식|중식|일식|양식|분식|주점|술집|호프|포차|치킨|피자|버거|족발|보쌈|곱창|고기|국밥|찌개|백반|국수|횟집|초밥|샤브|찜닭|닭갈비|김밥|도시락|식당|먹거리)/.test(leaf)) return true;
    // ⑤ 그 외(서점·공방·꽃집·장소대여 등) → 비카페
    return true;
  }
  // 카테고리 불명 → 이름 기반 보조
