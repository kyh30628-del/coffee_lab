...
// 🚨 재발방지(2026-07-26): 인천 2026-07-01 2군9구 개편(중구·동구→제물포구/영종구, 서구→검단구/서해구,
//   공식 인천시청 자료 확인) 전 목록이 옛 10개 구 그대로였다 — 즉 자율 발굴(cron-grow, 2시간마다)이
//   검단구·서해구·영종구·제물포구를 단 한 번도 검색한 적이 없었고, 이미 폐지된 "인천 동구" 등으로
...
export const DONGS: Record<string, string[]> = {
  ...
  // 인천 9개 구·군(2026-07-01 개편 반영) — 서울과 이름 안 겹치니 bare 키(DONGS[gu])로 충분.
  //   제물포구=옛 중구 육지부+동구 전체, 영종구=옛 중구 영종·무의도, 검단구·서해구=옛 서구 분리(아라뱃길 경계).
  // 🔧 2026-08-23 누락 보강: 경동·전동이 빠져 있어 이 권역 카페가 **발굴 검색 대상에서 제외**되고,
  //   area 판별도 못 해 오분류로 남아 있었다(좌표 37.47/126.63 = 동인천 육지부로 확인).
  // 🔧 2026-08-23 2차 보강(법률 기준 확인: 제물포구=옛 중구 내륙 + 동구 전체).
  //   ⚠️ '금곡동'은 넣지 않는다 — 동구 금곡동(제물포구)과 서구 금곡동(검단구)이 **동명이인**이라
  //     사전에 넣으면 판별이 갈린다. 판별 불가로 남겨 현재값을 보존하는 편이 안전하다.
  제물포구: ["신포동", "신흥동", "답동", "동인천", "개항로", "송월동", "송림동", "송현동", "화수동", "만석동", "금창동", "창영동", "배다리", "경동", "전동",
    "인현동", "율목동", "용동", "선린동", "북성동", "송학동", "화평동", "사동", "관동", "항동", "해안동", "중앙동"],
  // 🔧 2026-08-23: 덕교동 누락(좌표 37.43/126.42 = 용유·마시란 권역 = 영종도).
  // 🔧 2026-08-23: 영종구=영종도·용유도·무의도 일원(법률 기준). 을왕동·무의동·남북동 추가.
  ...
  // 🔧 2026-08-23: 검단구=아라뱃길 **북쪽**(법률 기준). 실측 좌표가 이 경계를 그대로 재현했다 —
  //   시천동 37.5710~37.5735(북) vs 검암동 37.5577~37.5674(남). 대곡동·오류동·백석동 추가.
  ...
  // 🔧 2026-08-23: 심곡동·신현동 누락(좌표 37.51~37.54/126.67~126.68 = 아라뱃길 남단 = 서해구).
  //   ⚠️ 심곡동·신현동은 부천시·광주시에도 동명이 있어, area 판정 시 **주소의 광역(인천)까지 함께 봐야** 한다.
  // 🔧 2026-08-23: 서해구=아라뱃길 **남쪽**. 검암동·경서동·신현원창동 추가(좌표로 남측 확인).
