...
// 🔁 2026-09-07 재정정: 인천 2026-07-01 2군9구 개편(제물포구·영종구·검단구·서해구)은 **실재**한다
//   (인천시 공식 incheon.go.kr/IC01070101 + 네이버 로컬 실측이 신구명 주소 반환). 08-31 "환각" 판정(#910)이
//   과잉 정정이었다 — SIDO_GU(단일출처)를 공식 목록으로 재복원. 발굴은 여기서 자동 파생돼 신설구를 다시 훑는다.
...
// 📤 export(2026-08-23): 인천 area 오분류 교정 스크립트가 이 표를 **단일출처**로 쓴다.
//   같은 매핑을 스크립트에 복붙하면 개편 반영이 갈라진다 — 반드시 여기서 가져다 쓸 것.
export const DONGS: Record<string, string[]> = {
  ...
  // 인천 2군9구(2026-07-01 개편, 공식 incheon.go.kr/IC01070101 관할 그대로).
  //   제물포구·영종구·검단구·서해구는 전국 유일명이라 bare 키(폴백 조회가 잡는다).
  //   ⚠️ '금곡동'은 넣지 않는다 — 제물포구(옛 동구) 금곡동과 검단권 금곡동이 **동명이인**이라 판별이 갈린다.
  // 제물포구 = 옛 중구 내륙(신포·개항로 일원) + 옛 동구 전체(만석·화수·송림 일원)
  제물포구: ["신포동", "신흥동", "답동", "동인천", "개항로", "송월동", "경동", "전동", "인현동", "율목동", "용동", "선린동", "북성동", "송학동",
    "관동", "항동", "해안동", "중앙동",
    "송림동", "송현동", "화수동", "만석동", "금창동", "창영동", "배다리", "화평동", "사동"],
  // 영종구 = 옛 중구 섬지역(영종·용유·무의)
  영종구: ["영종도", "운서동", "을왕리", "을왕동", "용유동", "중산동", "운남동", "운북동", "덕교동", "무의동", "남북동", "영종동", "운정동"],
  ...
  // 검단구 = 옛 서구 북부(아라뱃길 북) / 서해구 = 옛 서구 남부(청라·루원시티·가정·석남·가좌 일원).
  //   ⚠️ 심곡동·신현동은 부천시·광주시에도 동명이 있어, area 판정 시 **주소의 광역(인천)까지 함께 봐야** 한다.
  검단구: ["검단", "원당동", "당하동", "불로동", "마전동", "왕길동", "시천동", "대곡동", "오류동", "백석동", "아라동"],
  서해구: ["청라", "가정동", "석남동", "연희동", "루원시티", "가좌동", "원창동", "심곡동", "신현동", "검암동", "경서동", "신현원창동", "공촌동"],
  강화군: ["강화읍", "길상면", "화도면", "불은면", "선원면", "교동면"],
  옹진군: ["북도면", "백령면", "연평면", "덕적면", "영흥면", "자월면"],
  // 🏔️ 강원 18개 시군(2026-08-25 확장) — **여기가 수집 정확도의 핵심이다.**
  //   기존 강원 43곳은 '가평군'·'옹진군' 검색에 딸려 들어온 부수 수확이라 지역 컨텍스트 없이
  //   이름만으로 후기를 모았고, 그 결과 거절 사유 1위가 "카페명이 글에 없음(무관/동명)"이었다
  //   (모어아워 363건→노출 0 · 빵이당 461건→0). 흔한 이름은 이름만으로 검색하면 무관 글만 쌓인다.
  //   '춘천시 효자동 로스터리'처럼 좁혀야 진짜 그 카페 글이 잡힌다.
  //   ⚠️ 관광 명소가 아니라 **생활권 동**을 우선 나열했다(도심·대학가·아파트단지). 관광지는 자연히 잡힌다.
  춘천시: ["효자동", "석사동", "퇴계동", "후평동", "온의동", "약사동", "교동", "소양로", "우두동", "동면", "신북읍", "남산면"],
  원주시: ["무실동", "단계동", "명륜동", "일산동", "우산동", "반곡동", "관설동", "태장동", "지정면", "문막읍"],
  강릉시: ["교동", "포남동", "옥천동", "노암동", "입암동", "송정동", "초당동", "안목", "경포", "주문진읍", "사천면", "연곡면"],
  동해시: ["천곡동", "발한동", "북삼동", "묵호동", "부곡동", "삼화동"],
  태백시: ["황지동", "장성동", "철암동", "상장동"],
  속초시: ["교동", "조양동", "동명동", "청호동", "노학동", "대포동"],
  삼척시: ["교동", "정라동", "성내동", "남양동", "도계읍", "근덕면"],
  홍천군: ["홍천읍", "남면", "서면", "북방면", "화촌면"],
  횡성군: ["횡성읍", "우천면", "안흥면", "둔내면"],
  영월군: ["영월읍", "주천면", "김삿갓면", "한반도면"],
  평창군: ["평창읍", "대관령면", "진부면", "봉평면", "용평면"],
  정선군: ["정선읍", "고한읍", "사북읍", "임계면"],
  철원군: ["갈말읍", "동송읍", "철원읍", "김화읍", "서면"],
  화천군: ["화천읍", "사내면", "간동면", "상서면"],
  양구군: ["양구읍", "국토정중앙면", "동면", "해안면"],
  인제군: ["인제읍", "북면", "기린면", "서화면"],
  고성군: ["간성읍", "거진읍", "토성면", "죽왕면", "현내면"],
  양양군: ["양양읍", "강현면", "손양면", "현북면", "서면"],
};
...
// 🗺️ area **판정 전용** 지역 — 주소는 해석하되 발굴은 하지 않는 곳.
//
// 왜 METRO_REGIONS와 나눠 두나(2026-08-25 교훈): 그 배열은 두 가지 일을 겸한다.
//   ① parseGuArea가 주소→area를 잡는 사전  ② cron-grow가 discovery_state에 넣는 **발굴 대상 목록**.
//   그래서 "이미 DB에 있는 카페의 area가 틀렸다"는 이유로 한 줄 등재하면 그 지역 신규 발굴까지 자동으로 켜진다.
//   ①은 주소 해석(부작용 없음), ②는 돈과 쿼터를 쓴다 — 성격이 다르니 목록도 나눈다.
// 현재는 비어 있다(강원이 발굴 대상으로 승격). 앞으로 '주소만 바로잡고 싶은' 지역은 여기에 넣는다.
const AREA_ONLY_REGIONS: { region: string; areaLabel: string }[] = [];
...
// 🆕 2026-09-17 — 이 접두는 더는 '나쁜 주소'가 아니다. 광주·전남을 편입했으므로 **정규화 대상**이다.
//   실측: "전남광주통합특별시" 아래에 광주 구(동/서/남/북/광산)와 전남 시군(광양·여수·순천…)이 함께 온다.
//   접두를 떼고 남은 시군구 이름으로 광주/전남을 가른다(아래 MERGED_GJ_JN).
const BAD_SIDO_PREFIX = /^(전남광주통합특별시|전라남도광주|광주전남통합특별시)\s*/;
/** 통합 접두 뒤 시군구가 광주 자치구면 '광주', 아니면 '전남'. */
...
// 🆕 2026-09-17 전국 개방 — 범위 밖 시·도가 없다. 정규식을 '절대 매치 안 됨'으로 둔다
//   (분기·로그 경로는 살려 둔다 — 다음에 범위를 좁힐 때 여기만 되돌리면 된다).
const OUT_OF_SCOPE_SIDO = /^(?!)/;
const IN_SCOPE_SIDO: [RegExp, string][] = [
  ...
  // 🆕 2026-09-17 전국 개방. ⚠️ '광주'는 **광주광역시로만** 판정한다 — 경기도 광주시가 실제로 있다.
  [/^광주광역시/, "광주"], [/^울산(광역시)?/, "울산"],
  [/^전라남도|^전남(?!광주)/, "전남"], [/^전라북도|^전북(특별자치도)?/, "전북"],
  [/^제주(특별자치도|도)?/, "제주"],
...
export function addressSidoScope(addressRaw: string): { scope: "in" | "out" | "unknown"; sido?: string } {
  ...
  // 🆕 2026-09-17 — 통합 접두는 이제 범위 안이다. 접두를 떼고 시군구로 광주/전남을 가른다.
  const mg = raw0.match(BAD_SIDO_PREFIX);
  if (mg) {
    const gu = (raw0.slice(mg[0].length).trim().split(/\s+/)[0] ?? "");
    return { scope: "in", sido: GWANGJU_GUS.has(gu) ? "광주" : "전남" };
  }
  const a = raw0.trim();
...
//     도로명만 있는 주소(세종 고운한옥1길)는 동이 없어 안 잡힌다 — 정상이다. 있는 것만 더한다.
  ...
  // 🗺️ 수도권 박스 밖(좌표 있고 수도권 밖)만 제외 — 네이버가 동명 타지역 업체(성심당=대전 등)를 반환해도 서비스와 무관.
  //   좌표 null은 원본대로 통과(이름 대조). 공개 게이트도 막지만 적재 단계서 차단=DB 청결.
  const proc = found.filter((it) => !(it.lat != null && it.lng != null && !(it.lat >= latMin && it.lat <= latMax && it.lng >= lngMin && it.lng <= lngMax)));
  oob = found.length - proc.length;
  if (proc.length) {
    // 🚀 중복검사 배치화(2026-07-09) — 발굴카페마다 개별 SELECT(지역당 수백 왕복)를 '이름 ANY + 좌표박스' 1회 조회로.
    //   의미 무변(name OR coord-near 동일 판정), 스윕 속도 5~10× → 매 회차 전 지역 커버 가능.
    const names = [...new Set(proc.map((it) => it.name))];
    const withCoord = proc.filter((it) => it.lat != null && it.lng != null);
    let ex: any[];
    if (withCoord.length) {
