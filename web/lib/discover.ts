...
// ⚠️ 2026-08-31 정정(decisions#910): 과거 이 자리엔 "인천 2026-07-01 2군9구 개편(제물포구·영종구·
//   검단구·서해구)" 서술이 있었으나 그 개편 자체가 실존하지 않는 환각이었다(coordination#354).
...
export const DONGS: Record<string, string[]> = {
  ...
  // 인천 10개 구·군. ⚠️ 인천 중구는 서울 중구(위)와 bare 키가 충돌해 "인천 중구"처럼 full-region
  //   키로 넣는다(line ~415: DONGS[region] 우선 조회가 이미 이 패턴을 지원). 동구는 안 겹치지만
  //   대칭을 위해 동일하게 full-region 키로 넣는다.
  // ⚠️ 2026-08-31 정정(decisions#910): 이 자리에 있던 "제물포구·영종구·검단구·서해구"(2026-07-01 개편)는
  //   실존하지 않는 구명이었다(coordination#354) — 실제 인천 행정구역 기준으로 되돌린다.
  //   중구=구도심 내륙(신포·개항로 일원)+영종·무의도(법정동 기준 모두 중구), 동구=구도심 나머지(만석·화수 일원),
  //   서구=검단·청라·루원시티·아라뱃길 남북 전역(예전 검단구·서해구로 갈렸던 동은 전부 서구로 합친다).
  //   ⚠️ '금곡동'은 넣지 않는다 — 동구 금곡동과 서구 금곡동이 **동명이인**이라 사전에 넣으면 판별이 갈린다.
  "인천 중구": ["신포동", "신흥동", "답동", "동인천", "개항로", "송월동", "경동", "전동", "인현동", "율목동", "용동", "선린동", "북성동", "송학동",
    "관동", "항동", "해안동", "중앙동",
    // 영종·용유·무의도(법정동 기준 중구 소속)
    "영종도", "운서동", "을왕리", "을왕동", "용유동", "중산동", "운남동", "운북동", "덕교동", "무의동", "남북동", "영종동", "운정동"],
  "인천 동구": ["송림동", "송현동", "화수동", "만석동", "금창동", "창영동", "배다리", "화평동", "사동"],
  ...
  // 검단(아라뱃길 북)+청라·루원시티·가정·석남·가좌·원창·심곡·신현·검암·경서·공촌(아라뱃길 남) 전역 — 전부 서구.
  //   ⚠️ 심곡동·신현동은 부천시·광주시에도 동명이 있어, area 판정 시 **주소의 광역(인천)까지 함께 봐야** 한다.
  서구: ["검단", "원당동", "당하동", "불로동", "마전동", "왕길동", "시천동", "대곡동", "오류동", "백석동", "아라동",
    "청라", "가정동", "석남동", "연희동", "루원시티", "가좌동", "원창동", "심곡동", "신현동", "검암동", "경서동", "신현원창동", "공촌동"],
