...
export const isNonCafe = (name: string, category: string) => {
  ...
  if (cat) {
    // ① 음식점·주점 계열 카테고리 → 비카페(가장 명확)
    if (/(음식점|한식|중식|일식|양식|아시아음식|분식|주점|술집|호프|포차|뷔페|패스트푸드|치킨|피자|버거|족발|보쌈|곱창|고기|국밥|찌개|백반|국수|면요리|횟집|초밥|샤브|찜닭|닭갈비|김밥|도시락)/.test(cat)) return true;
    const leaf = (cat.split(">").pop() || "").trim();
    // ② 리프가 간식·디저트 '단품 전문'(커피 카페 아님) → 제외
    if (/(와플|토스트|핫도그|샌드위치|샐러드|아이스크림|빙수|젤라또|요거트|츄러스|크레페|타코야끼|붕어빵|호떡|꽈배기|어묵|떡집|방앗간|반찬|정육|청과)/.test(leaf)) return true;
    // ③ 명확한 카페류 리프 → 통과(보호)
    if (/(카페|커피|로스터|찻집|티하우스|디저트카페|베이커리카페|브런치카페|북카페)/.test(leaf)) return false;
    // ④ 베이커리·디저트·브런치(카페형 다수) → 보수적 통과
    if (/(베이커리|제과|디저트|브런치|도넛|케이크)/.test(leaf)) return false;
    // ⑤ 카테고리 있는데 카페류 아님 → 비카페(서점·공방·장소대여·꽃집…)
    return true;
  }
  // 카테고리 불명 → 이름 기반 보조
