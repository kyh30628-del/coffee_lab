...
export const METRO_SIGNGU: { code: string; areaLabel: string }[] = [
  ...
  // 인천 — 2026-07-01 2군9구 개편(중구·동구→제물포구/영종구, 서구→검단구/서해구, 공식 확인)으로
  // 옛 중구(28110)·동구(28140)·서구(28260) 코드는 이미 폐지된 시군구를 가리켜 제거함(써도 빈 응답
  // 또는 오류 위험). 🚧 제물포구·영종구·검단구·서해구의 신규 5자리 법정동코드는 미확정 — 이 기능은
  // DATA_GO_KR_KEY 자체가 로컬에 없어 현재 완전 비활성(크론 미연결, dry-run만 가능)이라 급하지 않음.
  // 활성화 전 반드시 행정표준코드관리시스템(code.go.kr) 또는 data.go.kr StanReginCd API로 실제 코드
  // 확인 후 채울 것 — 추측 코드 절대 넣지 말 것(잘못된 코드는 조용히 빈 결과만 반환해 발견이 어려움).
  ["28177", "인천 미추홀구"], ["28185", "인천 연수구"], ["28200", "인천 남동구"], ["28237", "인천 부평구"], ["28245", "인천 계양구"], ["28710", "강화군"], ["28720", "옹진군"],
  // 경기 (시 단위)
