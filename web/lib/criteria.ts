...
export const META: CriterionMeta[] = [
  ...
  // 🆕 2026-09-17 — 위 문턱을 낮출 때 짝으로 쓰는 신선도 조건(CEO 승인).
  //   08-29에 문턱을 3→5로 올린 진짜 이유는 "3건이 부족해서"가 아니라 **60일+ 낡은 수집 데이터로 3건을 세서**였다
  //   (실측: 3건짜리 193곳 재수집 시 48곳(24%) 하락 · 1,635곳 중 843곳이 60일+ 낡은 데이터 판정).
  //   문턱을 올리는 건 증상 대응이었다. 원인은 데이터 나이다 → 나이를 직접 건다.
  //   0이면 조건 없음(옛 동작). 실측 09-17: 자격 후보 1,209곳 중 1,173곳(97%)이 이미 30일 이내라 손실이 거의 없다.
