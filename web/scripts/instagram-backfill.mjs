...
// coord#245 후속(#493, H15): 스타필드·터미널 등 다중테넌트 몰에서 오귀속 재발 — 후보명도 식별토큰을 뽑아
// '겹치는 식별토큰이 실제로 있는지'까지 요구하도록 강화.
// decisions#780: nameMatches 자체 사본 유지가 lib/discover.ts의 별도(좌표전용) 백필 경로를 검증 없이 방치하는
// 근본원인이었다(#655 리셋 4~6일 뒤 재오염) — brandTokenOverlap(lib/reviewQuality.ts)로 단일화.
const nameMatches = (cafeName, areaTerms, candidateName) => brandTokenOverlap(cafeName, candidateName, areaTerms);
