...
// 카페당 1쿼리, 좌표 근접 매칭(이름 매칭만으론 오매칭 위험 — venue-token 오염 사례 참고)으로 정확도 확보.
...
try {
  while (processed < MAX && !stop) {
    ...
    const rows = await sql`
      SELECT id, name, area, lat, lng FROM cafes
    ...
    for (const c of rows) {
      ...
      try {
        ...
        // 좌표 근접 + link가 실제 instagram.com일 때만 채움(이름 매칭만으론 동명·옆가게 오염 위험).
        const m = res.items.find((it) => it.lat != null && Math.abs(it.lat - c.lat) < 0.0015 && Math.abs(it.lng - c.lng) < 0.0015 && IG_RE.test(it.link));
        if (m) {
