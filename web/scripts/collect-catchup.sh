...
# 🔴 2026-09-19 실측 — 크론 예약 2,000콜은 9배 과다였다.
#   cron-closure 실측 회당 **213콜**(150곳 확인, 곳당 1.4콜)이고 하루 4회여도 852콜.
...

{
  ...
  # 🤖 pending 판정 자동 청산(2026-09-21 CEO 승인) — 하루 상한 $1.5, pending만, 수거/제출 분리. 임베딩보다 먼저(판정 → 임베딩 → 공개).
  node --import tsx scripts/judge-pending.mjs --cap=1.5 2>&1 | tail -6 || true
