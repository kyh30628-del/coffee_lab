...
<div class="foot">소비자의 마인드로 사고하고, 소비자 경험을 최우선한다 · 5분마다 자동 새로고침</div>
  ...
  try {
    ...
    if (last[0] && last[0].executive_md === execMd) {
      await sql`UPDATE org_briefings SET token_today=${tokJ}::jsonb, crons=${cronJ}::jsonb, metrics=${metJ}::jsonb, delegated=${delJ}::jsonb, approvals=${apprJ}::jsonb, work_order=${workOrder}, meeting=${meetingMd} WHERE id=${last[0].id}`;
      console.log("↻ 보고서 내용 동일 — 중복 저장 안 함, 라이브 지표·업무지시·회의록만 갱신");
    } else {
      await sql`INSERT INTO org_briefings (created_at, executive_md, approvals, token_today, crons, metrics, delegated, work_order, meeting)
        VALUES (${execMtime.toISOString()}, ${execMd}, ${apprJ}::jsonb, ${tokJ}::jsonb, ${cronJ}::jsonb, ${metJ}::jsonb, ${delJ}::jsonb, ${workOrder}, ${meetingMd})`;
