...
const ids = await sql`SELECT id, name, area FROM cafes WHERE published AND synth_reviews_all IS NULL AND raw_reviews IS NOT NULL ORDER BY synth_count DESC NULLS LAST`;
console.log(`미생성 ${ids.length}곳 처리`);
let done = 0;
async function one(c) {
  try {
    ...
    const texts = raw.map(r=>({text:r.desc??r.text??r.quote??"",title:r.title??"",link:r.link??null,date:r.date??null,source:r.source??"naver"})).filter(t=>t.text?.length>5);
    ...
    if (done%300===0) console.log(`  …${done}/${ids.length}`);
  } catch {}
}
