...
async function build() {
  ...
  const rows = await sql`
    ...
    WHERE (c.published OR c.pipeline_status = 'held') AND c.synth_identity IS NOT NULL
      ...
      -- ⚠️ 일치율(synth_coherence)로 그라운딩을 건너뛰면 안 됨: 일치율은 '이름 토큰'만 봐서 지역어·지점
      --   혼동 오염(예: 봉명동내커피 금천독산점=일치율1.0인데 근거가 전부 미용실·던킨)에는 눈멀어 있음.
      --   그라운딩이 바로 그걸 잡으므로 coherence 게이트 적용 금지. 효율은 '1회 검사 후 캐시(미변경 재검 안 함)'로만.
