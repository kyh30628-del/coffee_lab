...
- about=false: 다른 가게·동명 카페·무관한 글(단순히 단어만 우연히 겹침), 나열식 맛집 모음에 이름만 끼인 글.
- helpful=true: 맛·분위기·메뉴·서비스 등 구체적 경험/평가가 담겨 소비자에게 도움됨. false: 광고·협찬 위주, 내용 없는 단순 언급, 사진만.
반드시 JSON 배열로만 답한다(설명·코드블록 금지): [{"i":번호,"about":true/false,"helpful":true/false}]`;
...
async function apply(cafeId, whitelistKeys) {
  const r = await fetch(`${APP_URL}/api/judge-apply`, { method: "POST", headers: { "Content-Type": "application/json", "x-admin-password": PW }, body: JSON.stringify({ cafeId, whitelistKeys }) });
  return r.json();
}
...
async function main() {
  ...
  while (done < MAX_CAFES) {
    ...
    if (cafes.length === 0) { console.log(`경계 리뷰 카페 없음(스킵 ${noBorderline}). 남은 후보 ${remaining}.`); if (remaining === 0) break; continue; }
    for (const c of cafes) {
      const verdicts = await judge(c.name, c.area, c.borderline);
      let keys = [];
      if (Array.isArray(verdicts)) keys = verdicts.filter((v) => v?.about && v?.helpful).map((v) => c.borderline[v.i]?.key).filter(Boolean);
      const res = await apply(c.cafeId, keys);
      if (res?.published) published++;
      done++;
      console.log(`[${done}] ${c.name}: 경계 ${c.borderline.length} → 양질 ${keys.length} | 등급 ${res?.grade ?? "?"} 공개 ${res?.published ?? "?"}`);
