    ...
    <div role="button" tabIndex={0} onClick={() => onOpen(c.id)} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); onOpen(c.id); } }} className={`w-full text-left nt-scrap mb-4 px-4 pt-4 pb-3.5 cursor-pointer ${featured ? "featured" : ""}`} style={{ ["--rot" as any]: tone % 2 ? "0.35deg" : "-0.35deg" }}>
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="nt-eyebrow mb-1" style={{ color: "#a0672a" }}>{kicker}</div>
...
function memoRuns(line: string): { h: 0 | 1 | 2; chars: { ch: string; ci: number }[] }[] {
  const runs: { h: 0 | 1 | 2; chars: { ch: string; ci: number }[] }[] = [];
...
function memoChars(line: string): { ch: string; h: 0 | 1 | 2 }[] {
  const out: { ch: string; h: 0 | 1 | 2 }[] = []; let h: 0 | 1 | 2 = 0;
  for (const ch of Array.from(line)) { if (ch === "【") h = 1; else if (ch === "〔") h = 2; else if (ch === "】" || ch === "〕") h = 0; else out.push({ ch, h }); }
  return out;
}
    ...
    <div className="w-full max-w-md mx-auto flex flex-col nt-landing" style={{ height: "100%", overflowY: "auto", overflowX: "hidden", background: "var(--nt-espresso)", padding: "calc(env(safe-area-inset-top) + 8px) 8px calc(env(safe-area-inset-bottom) + 8px)", boxSizing: "border-box" }}>
      ...
      <div ref={heroRef} className="relative w-full overflow-hidden min-h-[220px]" style={{ flex: "0 1 auto", minHeight: 220, aspectRatio: `${HERO_W} / ${HERO_H}`, background: "var(--nt-espresso)" }}>{/* 2026-09-22 CEO("노트 위 공백"): 히어로는 그림 높이만큼만(제목은 그림 위 나무 위에 얹힘). 남는 높이는 아래 CTA 블록이 고르게 나눠 갖는다. 짧은 화면은 flex-shrink로 줄고 calc가 축소 */}
        <div className="absolute bottom-0" style={heroBox ? { width: heroBox[0], height: heroBox[1], left: heroBox[2] } : { left: 0, width: "100%", aspectRatio: `${HERO_W} / ${HERO_H}` }}>
          <img src="/note/hero14.webp" alt="" aria-hidden className="block w-full h-full" />
        ...
        <div ref={pageRef} className="absolute left-0 top-0" style={{ width: PAGE_SW, height: PAGE_SH, transformOrigin: "0 0", transform: mtx || "translate(-9999px,0)" }}>
          ...
          <div className="absolute" style={{ left: 42, right: 10, top: lineTop }}>
            {LANDING_MEMO.map((line, li) => (
              <div key={li} className={`nt-w nt-hand${li === 0 ? " date" : ""} ${allDone || (done === false && li < pos[0]) ? "done" : ""}`} style={{ fontSize: 22, lineHeight: `${PAGE_PITCH}px`, height: PAGE_PITCH, color: "#2f3550", whiteSpace: "nowrap", overflow: "hidden" }}>
                ...
                {done !== null && /다시 갈|좋았다/.test(line) && <i className="nt-star" aria-hidden />}
              </div>
            ))}
          </div>
      ...
      <div className="px-4 -mt-2 relative z-[1] flex flex-col justify-evenly" style={{ color: "#f6ecdf", paddingBottom: 10, flex: "1 0 auto" }}>{/* 남는 높이를 문단·버튼 사이에 고르게 — 한 덩어리 빈 띠가 안 생긴다 */}
        ...
        <div className="space-y-2 mt-3 max-w-md mx-auto">
          ...
          <button onClick={onOwner} className="w-full rounded-lg py-3 px-5 text-left flex flex-col gap-0 active:scale-[0.99] transition" style={{ background: "rgba(255,255,255,.06)", border: "1px solid rgba(233,214,189,.45)", color: "#f6ecdf" }}>
          ...
          <button onClick={onLogin} className="block w-full text-center text-[12px] underline underline-offset-2 pt-1" style={{ color: "#c9b391" }}>
            이미 키가 있어요 · 로그인
          </button>
        </div>
        <div>
...
export default function Home() {
  ...
  return (
    <div className="flex flex-col nt-paper nt-app" style={{ position: "fixed", inset: 0, fontFamily: "var(--nt-font)" }}>
      ...
      {/* 홈 = 잡지 1면 */}
      {tab === "home" && (
        <div className="flex-1 overflow-y-auto overflow-x-hidden" style={{ paddingBottom: "3.25rem", position: "relative" }}>
      ...
      <nav className="md:hidden flex items-stretch nt-header" style={{ position: "fixed", left: 0, right: 0, bottom: 0, height: "3.25rem", zIndex: 1300, boxShadow: "0 -2px 10px rgba(30,18,10,.28)" }}>
        {[
          ...
          const color = a.active ? "#f0d3a5" : "#c9b391";
          return (
          ...
          }} className="flex-1 flex flex-col items-center justify-center active:bg-[#3a2a1f]" aria-label={a.label} aria-current={a.active ? "page" : undefined}>
            <div className="flex flex-col items-center justify-center gap-0.5 rounded-2xl px-4 py-1 transition-colors" style={{ background: a.active ? "rgba(255,255,255,.10)" : "transparent" }}>
