...
import { applyDecisions, markJudged } from "@/lib/synthStore";
...
export async function POST(req: NextRequest) {
  try {
    ...
    // decisions: {리뷰key: keep(true)/drop(false)} — Sonnet 최종 판정
    const decisions: Record<string, boolean> = (body.decisions && typeof body.decisions === "object") ? body.decisions : {};
    ...
    if (Object.keys(decisions).length === 0) { await markJudged(cafeId); return NextResponse.json({ ok: true, id: cafeId, approved: 0, marked: true }); }

    const result = await applyDecisions(cafe, decisions);
