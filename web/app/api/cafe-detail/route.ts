...
export async function GET(req: NextRequest) {
  try {
    ...
    const rows = await sql`SELECT name, area, dong, COALESCE(synth_reviews_all, synth_reviews) AS synth_reviews_all, synth_quality, llm_judged_at, reputation_note, cautions, address, instagram_url, review_dates FROM cafes WHERE id=${idNum} AND published=true LIMIT 1`;
    ...
    return NextResponse.json({ ok: true, area: rows[0]?.area ?? null, reviews, quality, llmJudged, highlights, cautions, reputationNote: rows[0]?.reputation_note ?? null,
      // 📓 상세 패널 2차(2026-09-20): 주소·인스타·후기 날짜 — 같은 행의 작은 컬럼 3개(조회 추가 0).
      address: rows[0]?.address ?? null, instagramUrl: rows[0]?.instagram_url ?? null, reviewDates: Array.isArray(rows[0]?.review_dates) ? rows[0].review_dates : null }, {
