...
//   서구 폐지→검단구·서해구로 분리, 공식 인천시청 자료 확인)됐는데 목록엔 옛 10개 구만 있어 134곳이
//   "인천"으로만 뭉쳐지고 있었다. 옛 중구·동구·서구 소속 카페 263곳도 법정동 기준(영종·무의도 8개동→
//   영종구, 나머지 중구/동구→제물포구, 검단 옛 김포편입분 10개동→검단구, 나머지 서구→서해구)으로 실제
//   재분류 완료 — 옛 구 이름을 쓰는 카페가 이제 없어 목록에서도 제거. 예전엔 "동구"⊂"남동구" 부분문자열
