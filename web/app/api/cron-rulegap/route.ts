...
export async function GET(req: NextRequest) {
  try {
    ...
    const okIds = new Set(((await sql`SELECT term FROM learned_terms WHERE kind='cafe_ok' AND status='active'`) as { term: string }[]).map((r) => Number(r.term)));
    ...
    for (const c of catCafes) {
      ...
      if (okIds.has(c.id)) continue;                       // 사람이 검토·승인(유지)한 카페 — 재플래그 안 함
