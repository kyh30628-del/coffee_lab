...
import { getRegions, getRegionFacetCafes, getRegionFacetCount, getRegionFacetCounts, getRegionFacetGradeBreakdown, areaAliases, SITE } from "@/lib/seoData";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const desc = `${areaShort ? `${areaShort}·` : ""}${area}에서 ${f.aliases.slice(0, 2).join(", ")}를 찾는다면. ${f.desc} 카페 ${total}곳을 영수증 리뷰·광고 없이 진짜 후기로 검증해 골랐어요.${names ? ` ${names} 등.` : ""}`;
...
export default async function RegionFacetPage({ params }: Props) {
  ...
  const [cafes, regions, total, grades, allCounts] = await Promise.all([
    ...
    getRegionFacetGradeBreakdown(area, f.label), getRegionFacetCounts(),
  ]);
  ...
  return <Curated area={area} tasteLabel={f.title} tasteEmoji={f.emoji} heading={heading} intro={intro}
    cafes={cafes} regions={regions} grades={grades} tasteCounts={{}} sameTasteNearby={sameNearby}
