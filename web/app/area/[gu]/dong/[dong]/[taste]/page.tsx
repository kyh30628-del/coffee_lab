...
import { getDongTasteCafes, getDongTasteCounts, getDongsInArea, tasteByKey, TASTES, SITE, TASTE_MIN_HITS, TASTE_MIN_RATE_PCT } from "@/lib/seoData";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const cafes = await getDongTasteCafes(area, d, taste, 30);
  const names = cafes.map((c) => c.name).slice(0, 3).join(", ");
  const bestN = cafes.length >= 5 ? ` BEST ${cafes.length}` : "";
  ...
  const desc = `${area} ${d}에서 ${t.aliases.slice(0, 2).join(", ")}를 찾는다면. 검증 후기 ${ev.toLocaleString()}건을 근거로 ${t.desc} 카페를 영수증 리뷰·광고 없이 골랐어요.${names ? ` ${names} 등.` : ""}`;
...
export default async function DongTastePage({ params }: Props) {
  ...
  const heading = `${d} ${t.label} 카페${cafes.length >= 5 ? ` BEST ${cafes.length}` : ""}`;
  const intro = `${d} ${t.aliases.slice(0, 2).join("·")} 찾으시나요? ${area} ${d}에서 ${t.desc} 카페 ${cafes.length}곳. 후기에 ${t.short} 이야기가 ${TASTE_MIN_HITS}건 이상, 그 카페 전체 후기의 ${TASTE_MIN_RATE_PCT}% 이상 나온 곳만 골랐어요.`;
