...
//
// q= 쿼리파라미터(2026-09-16): /owner/r/[id] 무료 리포트에서 "🏅 배지 달기"로 들어오면
//   카페명을 미리 채우고 검색까지 자동 실행한다 — 수동 재입력 없이 바로 미리보기.
function BadgeForm() {
  ...
  const search = async (query?: string) => {
    ...
    const d = await fetch(`/api/search?q=${encodeURIComponent(term)}`).then((r) => r.json()).catch(() => null);
...
export default function BadgePage() {
  return (
    <main className="min-h-screen nt-paper nt-app text-[#17100b] px-5 py-10" style={{ fontFamily: "'DCN Hand', 'Nanum Pen Script', 'Apple SD Gothic Neo', sans-serif" }}>
      <Suspense fallback={<div className="max-w-lg mx-auto text-[13.5px] text-[#7a6a55]">불러오는 중…</div>}>
        <BadgeForm />
      </Suspense>
    </main>
  );
