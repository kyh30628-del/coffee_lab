"""export_cost.py — 원가+시세신호+브라우저 재계산용 원본(산지별 관세 포함) 내보냄"""
import csv, json
...
def run():
    ...
    payload = {
        ...
        "lb_to_kg": LB_TO_KG,
        "origins": [{"origin": o, "point_diff": p, "tariff": t, "note": n}
                    for o, p, t, n in origins],
        "default_assumptions": ASSUMPTIONS,
        "signal": price_signal(),
        "rows": rows,
    }
    OUT.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"내보내기 완료: {OUT}  (산지 {len(rows)}개, 산지별 관세 적용)")
